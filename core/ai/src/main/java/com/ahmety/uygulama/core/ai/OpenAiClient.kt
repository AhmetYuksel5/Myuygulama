...
class OpenAiClient @Inject constructor(
    ...
    private fun passageInstruction(): String = buildString {
        ...
        append("e (array of 1-3 notes in Turkish. Do not merely name the ")
        append("difficulty — teach it, so the reader can decode the NEXT sentence ")
        append("like it on their own. Say what the expression literally says, ")
        append("where it comes from, when an English speaker reaches for it, and ")
        append("what register it belongs to (slang, formal, old-fashioned). ")
        append("Grammar labels alone (\"present perfect kullanılmış\") teach ")
        append("nothing; explain what that choice DOES to the meaning. ")
        append("Never write a note about inversion or word order; it is not what ")
        append("stops a reader. Say nothing rather than filling the list), ")
